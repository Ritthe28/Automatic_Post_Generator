...
const AcconuntList = ({accounts, onDisconnect}:AccountListProps) => {
...
if (accounts.length===0) {
    return <div className="bg-white rounded-2xl border-2 border-dashed border-slate-200 flex flex-col items-center py-20 px-6">
        <div className="size-14 bg-slate-50 rounded-2xl flex items-center justify-center mb-4 border border-slate-100 opacity-50">
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">

      {
        accounts.map((account , index)=>{
            ...
            return (<div key={index} className="group bg-white border border-slate-200 rounded-2xl p-5 flex items-center gap-4 hover:border-slate-300 transition-all">
...
<span className="text-xs text-amber-600"> Disconnected </span>
      ...
      className="ml-2 p-1.5 rounded-lg text-slate-300 group-hover:text-red-500  transition-all"><UnplugIcon className="size-4"/></button>      </div>)
