...
<div className="p-6 flex flex-col gap-2">
{
  PLATFORMS.map((p)=>{
    ...
    return (
      <button key={p.id} disabled={isConnecting||isConnected} onClick={()=>onConnect(p.id)}
      className={`flex items-center gap-3 p-3.5 rounded-xl border text-left transition-all ${isConnected?"border-red-200 bg-red-50 cursor-default":"hover:bg-slate-100 cursor-pointer"} ${isConnecting&& "opacity-60"}`}
      >
        <div className="p-2">
