...
<div className="bg-white rounded-xl border border-slate-200 p-6">
  ...
  <form onSubmit={handleSchedule} action="" className="space-y-5">
{/* Platforms  */}
<div>
  ...
  <div className="flex flex-wrap gap-3">
    {
      PLATFORMS.map((p)=>{
        ...
        return(
          <button onClick={()=>togglePlatform(p.id)} type="button" className={`flex items-center gap-1.5 p-3 rounded-md border transition-all duration-150 ${active?"bg-red-50 border-red-300 text-red-500 scale-103":"border-slate-200 text-slate-500 hover:border-slate-300"}`}>
...
<div>
  <label htmlFor=""  className="block text-xs text-slate-500 uppercase mb-2">Content
  </label>
  <textarea name="" id="" rows={5} placeholder=""
...
<button type="button" onClick={()=>setMediaFile(null)} className="absolute top-2 ring-2 size-7 bg-slate-900/60 hover:bg-slate-900/80 text-white rounded-full flex items-center justify-center transition-colors">
...
<label className="flex items-center justify-center gap-2 p-5 py-10 border-2 border-dashed border-slate-200 rounded-xl cursor-pointer hover:border-red-300 hover:bg-red-50/30 transition-all group">
  ...
  <input  className="hidden" onChange={(e)=>e.target.files?.[0]&&setMediaFile(e.target.files[0])} type="file" accept="image/*, video/*" name="" id="file" />
</label>
  )  
  }
</div>
...
<input type="time" required className="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-slate-900 text-sm outline-none" value={scheduledDate} onChange={(e)=>setScheduledTime(e.target.value)} />
...
<button type="submit" disabled={loading} className="w-full flex  items-center justify-center gap-2 p-3.5 bg-red-500 hover:bg-red-600 transition-all text-white rounded-lg">
  {loading?
  (
<>
<div className="size-4 border-2 border-white border-transparent rounded-full animate-spin">
  Schedulling
</div>
</>
  )
    ...
    post.mediaType && <span className="text-xs bg-slate-100 text-slate-600 border border-slate-200 px-1.5 py-0.5 rounded-md font-semibold capitalize">
{
  ...
  <span className="text-xs bg-emerald-50 text-emerald-700 border border-e-mist-100 px-2 py-0.5 rounded-full">Published</span>
