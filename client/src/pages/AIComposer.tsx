...
const tones = ['Professional', "Creative", "ftruncateSync", "Minimilist", "Excited"]
  return (
    ...
    <div className=" max-w-4xl mx-auto space-y-12 pb-20 animate-in fade-in duration-700">
      ...
      <div className="space-y-6 text-center mt-20">
        ...
        <div className="relative group mt-12">
          <textarea name="" id="" placeholder="Share your idea... (e.g A post about the launch of our)"
          ...
          <div className="absolute bottom-4 right-2.5 flex items-center gap-3 text-sm">
            ...
            <button onClick={handleGenerate} className="bg-slate-900 hover:grid-cols-8 text-white flex items-center gap-2 px-4 py-2 rounded-lg">
              {
                loading ?
...

      {
        ...
        <div className="fixed inset-0 min-h-screen z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-md animate-in fade-in duration-300">
          <div className="bg-white rounded-xl shadow-2xl shadow-2xl w-full max-w-2xl border border-slate-100 overflow-hidden flex flex-col max-h-[90vh]">
...
<div className="bg-slate-50 rounded-2xl p-6 border border-slate-100 space-y-4">
  ...
  <img src={activeScheduler.mediaUrl} alt="pre" className="w-full aspect-video object-cover rounded-xl border border-slate-200 shadow-sm" /> }
</div>
</div>
...
<div className="space-y-6"> 
  <div>
    <label htmlFor="" className="block text-xs text-slate-600 uppercase tracking-widest mb-4"> Select Channel</label>
...
<div className="relative">
  ...
  <input type="date" className="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-100 rounded-md text-slate-900 text-sm focus:outline-none transition-all" name="" id="" value={scheduleDate} onChange={(e)=>setScheduleDate(e.target.value)} />
</div>
<div className="relative">
  <ClockIcon className="size-4 absolute left-4 top-1/2 -translate-y-1/2 text-slate-400"/>
  <input type="time" className="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-100 rounded-md text-slate-900 text-sm focus:outline-none transition-all" name="" id="" value={scheduleTime} onChange={(e)=>setScheduleTime(e.target.value)} />
</div>
  </div>
</div>
